...
-   use aspects such as hair style or color, attire, to identify a specific person [@odonnell_how_nodate];
...
In such cases, we suggest that such processing should be prohibited, as a form of criticalisation. Furthermore, it should be clarified that the exception under consent applies only when the consent is obtained from *both* parties: the individual whose data already contains sensitive characteristics, as well as the individual whose sensitive characteristics are being inferred.

> TODO: Rania - how about full groups/families? Is it even practical to get the conset of all in a famil yof 4-6 people?
...
2.  The massive amounts of data available for processing nowadays often means that an AI system is not explicitly inferring sensitive characteristics, but is rather adopting a proxy.\
    ...
    Therefore, profiling should be subject to further restrictions. In particular, prior to their usage in decisions that can negatively impact data subjects; and prior to their sharing with a third party, profiles should be tested for their correlation to sensitive characteristics, and where this is found to be true, their usage be prohibited.
...
There are different categories of AI derivative content. These categories can, under different circumstances, be considered *personal data*. The table below provides an overview of the different categories, the circumstances under which they could be considered personal data, and suggestions for compliance handling.
...
In order to protect data subjects' rights, some requirements may fall on the user of the AI system, rather than on the data processor.  For example, while the bulk processing of data can include sensitive characteristics, when it comes to a down-stream operator who wants to use the trained system to infer sensitive characteristics, this should be prohibited.  In addition, attempts to extract personal data from the model (re-identification) should also be forbidden.  The AI system user should also be responsible for de-criticalising any personal data generated by the system.


> TODO: Rania - warning right against harmful mention/no style cloning is currently technically impossible - consider this table as a draft - it may need refinement. Perhaps the table is not the right structure and we should proceed in the original linear heading based approach. DONE
> TODO Georg: I like the table structure.  I have added the technical feasibility warning directly in the table.  In addition:  for the "Embeddings" heading: Is it technically feasible to require de-criticalisation of bulk data used in training the model? Does "at generation time" = at inference time?  what is "embedding provenance"?  and are you saying it is the _user_ of the LLM who has to de-criticalise any generated output, and who is forbidden to use the LLM for re-identification?  For the "generative output" heading: who is the "up-stream" provider?  
Finally, I am not sure the "usage characteristics" belongs in the table.  I have pulled it out.

> - generation time: yes inference
> - embedding provenance is so far a  term not well defined - but probably should be related to normal data provenance (especially given that new paper)
> - I think *the user* is the whole point (concluding after the table) who is using the technology for good or bad and who has to have the right intentions and final oversight
> - agreed usage characteristics should be down

## fundamental rights - exceptions to article 9 GDPR


The meaning of draft Article 10 (1a) is ambiguous.
...
Presumably, interpretation 1 is intended?

In order to do bias testing, it is often necessary to infer sensitive characteristics and it is unclear, whether the exceptions provided in draft Article 10(2), apply also to prohibitions in the draft Article 5(2). In addition, such an exception would be necessary - not just for bias testing, but for determining other Fundamental Rights harms as well (for example, in order for CSOs and academia to conduct research on systemic risks of online platforms under DSA). Generally, such activities, while exempt from the prohibitions, should however be considered high risk.
...
Digital rights organisations studying social media for their human rights impacts often need to process billions of comments from millions of users. They may do this themselves, or by going through another provider, possibly an SME.  Thus, while the requirements on large scale controllers in Articles 7 and 8 are necessary for the protection of data subject rights, it is important to consider that these requirements may also hit SMEs and CSOs, and not just large organisations.
...
There exists no established platform for B2C for consent management. One notable implementation the authors are aware of is mydatamyconsent[^4]. Other tools mainly are there for deleting unwanted shared data like for example[^5]. Exchanging data for B2B purposes is even more important. To our best knowledge for sovereign data exchange only a limited set of solutions[^6] exists which tackles issues of identity and permission management in a holistic solution. Both examples for b2b and b2c function as isolated solutions rather than components of an interoperable, standardized ecosystem, thereby highlighting the existing architectural gap. One of the best examples for national data exchange and consent management may be found in Estonia. X Road is the name of of the data interchange system in Estonia. It is also the backbone for managing healthcare data there [@noauthor_estonian_2024]. One of the applications for consent managemnt running on top of X Road is Estfeed [@news_estfeed_2020].
...
Additionally, the concepts of "durable medium" (Article 7 (1b)) and "online interface" (Article 7 (1d)) needs to be further specified in the context of consent management.

> TODO: Rania this part was deleted - was it intentional? I think it should be somehow here - perhaps in other words. see the bullets
...
> TODO: Rania - I think mentioning the operating model of xroad and learning from that would be smart https://x-road.global/x-road-organizational-model
